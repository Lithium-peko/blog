...
// * 新增/更新 角色, 同时维护 role_menu, role_resource, casbin_rule 中的 关联数据
func (*Role) SaveOrUpdate(req req.SaveOrUpdateRole) (code int) {
	...
	dao.Delete(model.RoleMenu{}, "role_id", req.ID) // 删除 role_menu 中的 旧数据
	if len(req.MenuIds) > 0 {
		// 往 role_menu 中 添加新的数据
...
// * 根据 [id列表] 删除 role 中数据, 同时删除 role_menu, role_resource, casbin_rule 中的 关联数据
